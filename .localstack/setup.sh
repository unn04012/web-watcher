...
until curl -s http://localstack:4566/_localstack/health | grep -q '"sqs": "running"'
do
...
aws --endpoint-url=http://localstack:4566 sqs create-queue \
    ...
    --attributes VisibilityTimeoutSeconds=300,MessageRetentionPeriod=1209600,ReceiveMessageWaitTimeSeconds=20

# 2. Analysis Job Queue - LLM 분석 작업을 처리하는 큐  
aws --endpoint-url=http://localstack:4566 sqs create-queue \
    --queue-name analysis-jobs \
    --attributes VisibilityTimeoutSeconds=600,MessageRetentionPeriod=1209600,ReceiveMessageWaitTimeSeconds=20
